...
for fname in args.fnames:
    ...
    chivst = np.empty(
        0, dtype=[("t", "d"), ("amin", "d"), ("chi", "d"), ("chierr", "d")]
    )
    ...
    for it in trange:
        ...
        chi_tmp = np.array(
            [(it, a_min, chi, err_chi)],
            dtype=[("t", "d"), ("amin", "d"), ("chi", "d"), ("chierr", "d")],
        )
    ...
    plt.errorbar(t0_tmp_symE[0], s_TC_avg, yerr=s_TC_err, marker="o", color="r")

    # Plot the chi as a function of flow-time t
    plt.plot(chivst["t"], chivst["chi"], color="r", label=lab)
    plt.fill_between(
        chivst["t"],
        chivst["chi"] - chivst["chierr"],
        chivst["chi"] + chivst["chierr"],
        alpha=0.3,
        color="r",
    )
